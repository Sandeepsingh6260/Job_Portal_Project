...
import com.yourpackage.util.CloudinaryUtil;
...
public class JobSeekerServlet extends HttpServlet {
        ...
        try {
            ...
            Map uploadResult = cloudinary.uploader().upload(filePart.getInputStream(), 
                ObjectUtils.asMap(
                    ...
                    "public_id", "resume_" + jobSeeker.getId() + "_" + System.currentTimeMillis()
                ));
    ...
    private int calculateProfileStrength(JobSeeker jobSeeker) {
        ...
        if (jobSeeker.getFullName() != null && !jobSeeker.getFullName().trim().isEmpty()) strength += 15;
        if (jobSeeker.getEmail() != null && !jobSeeker.getEmail().trim().isEmpty()) strength += 15;
        if (jobSeeker.getPhone() != null && !jobSeeker.getPhone().trim().isEmpty()) strength += 15;
        if (jobSeeker.getLocation() != null && !jobSeeker.getLocation().trim().isEmpty()) strength += 10;
        if (jobSeeker.getSummary() != null && !jobSeeker.getSummary().trim().isEmpty()) strength += 20;
        if (jobSeeker.getSkills() != null && !jobSeeker.getSkills().trim().isEmpty()) strength += 15;
        
        // Check if resume exists
        Resume resume = resumeService.getResumeByJobSeekerId(jobSeeker.getId());
        if (resume != null) strength += 10;
